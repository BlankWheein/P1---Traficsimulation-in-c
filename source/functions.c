...
void prompt(double *thru_put, int *iter_speed, int *cars_sec, int *road_int, int *bus_lane_int, int *duration){
  ...
  do{
    system("cls");
    printf("\nHow often do you want the time interval between each vehicle?\nAverage time is around 30 seconds ");
    scanf(" %d", cars_sec);
  }while(*cars_sec < 0);

  do{
    system("cls");
    printf("\nHow many lanes in do you want in total? ");
    scanf(" %d", road_int);
  }while(*road_int < 1);
 ...
 * @retval None
 */
void save_to_file(Vehicle *cars, int cars_int,
                  int secs, int duration,
                  double throughput, int speed,
                  int cars_sec, int lanes, int plusbuslanes) {
  FILE * fp;
   int i;
   fp = fopen ("data.csv","w+");
   fprintf(fp,
   ...
   for(i = 0; i < cars_int; i++){
       fprintf(fp, "%d,%s,%d,%d,%lf,%d\n",
       cars[i].ID,lane_to_string(cars[i].type), cars[i].lane +1,
       cars[i].secs_on_bridge, cars[i].avg_speed, cars[i].time_waited_for_green_light);
   }
  fclose (fp);
}
